...
function View6({ setView }: Props) {
  ...
  const handleFieldChange = (field: string, value: string) => {
    ...
    switch (field) {
      case "activitiesToTry":
        setActivitiesToTry(value);
        break;
      case "resourcesForGoals":
        setResourcesForGoals(value);
        break;
  ...
  const createApplyFormDocument = async () => {
    try {
      const schemaName = "applyForm";
      ...
      // Navigate to the next view or perform the desired action
    } catch (error) {
  ...
  const handleNextClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const validationPromises = [
      validateField("activitiesToTry", activitiesToTry),
      validateField("resourcesForGoals", resourcesForGoals),
      validateField("additionalInformation", additionalInformation),
    ];
      ...
      <div className="flex flex-col lg:flex-wrap lg:flex-row lg:justify-between items-center gap-y-4 md:gap-y-6 w-full mt-8">
        ...
        <div className="w-full lg:w-[47%]">
          <h1 className="text-sm text-white font-normal md:text-lg">
            9. What type of activities would you like to try that you have never
            tried before (i.e. gardening, horse back riding, boxing, drumming,
            etc.)
          </h1>
          <div className="h-12 bg-transparent border-white border-opacity-60 border-[1px] rounded-lg w-full mt-2 md:mt-4 md:h-16">
            <input
              ...
              placeholder="Activities to Try"
              value={activitiesToTry}
              onChange={(e) =>
                handleFieldChange("activitiesToTry", e.target.value)
              }
            />
          </div>
          {errors.activitiesToTry && (
        ...
        <div className="w-full lg:w-[47%]">
          <h1 className="text-sm text-white font-normal md:text-lg">
            10. In a perfect world, what resources would be available to you to
            aid the success of the three goals you listed on #5? (i.e. therapy,
            tutoring, mentorship, etc.):
          </h1>
          <div className="h-12 bg-transparent border-white border-opacity-60 border-[1px] rounded-lg w-full mt-2 md:mt-4 md:h-16">
            <input
              className="outline-none w-[90%] overflow-hidden h-full text-xs text-white bg-transparent ml-2 md:ml-4 md:text-base resize-none"
              placeholder="Resources for Goals"
              value={resourcesForGoals}
              onChange={(e) =>
                handleFieldChange("resourcesForGoals", e.target.value)
              }
            />
          </div>
          {errors.resourcesForGoals && (
            <p className="text-xs text-red-500 mt-1">
              {errors.resourcesForGoals}
            </p>
          )}
        ...
        <div className="w-full lg:w-[47%]">
          <h1 className="text-sm text-white font-normal md:text-lg">
            Please attach any additional information or documents via links that
            you believe would enhance your application (i.e. art, music, or
            other etc. ):
          </h1>
          <div className="h-12 bg-transparent border-white border-opacity-60 border-[1px] rounded-lg w-full mt-2 md:mt-4 md:h-16">
            <input
              ...
              onChange={(e) =>
                handleFieldChange("additionalInformation", e.target.value)
              }
            />
          </div>
